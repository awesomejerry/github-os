...
import { fetchUserRepos, fetchRepoContents, fetchFileContent, repoExists, getRepoInfo, getCache, searchCode, fetchRepoCommits, fetchRepoBranches, fetchRepoTree, fetchRepoIssues, fetchRepoContributors, fetchRepoReleases, getFile, createFile, deleteFile, checkFileExists, getDefaultBranchSHA, createBranch, deleteBranch, clearBranchCache, batchCommit, createPR, mergePR, closePR, fetchPR, fetchIssue, createIssue, updateIssue, addIssueComment, fetchNotifications, markNotificationsRead, fetchWorkflowRuns, fetchWorkflowRun, fetchWorkflowJobs, fetchWorkflowLogs, rerunWorkflow, fetchWorkflows } from './github.js';
...
export const commands = {
  ...
  // Theme commands
  theme: cmdTheme,
  // Notifications & Actions commands
...
export async function getCompletions(githubUser, currentPath, partial) {
  ...
  if (parts.length === 1) {
    const commands = ['help', 'ls', 'cd', 'pwd', 'cat', 'tree', 'clear', 'exit', 
                      'whoami', 'connect', 'info', 'readme', 'head', 'tail', 'download', 'grep', 'log', 'branch', 'find', 'issues', 'contributors', 'releases', 'login', 'logout', 'status', 'touch', 'mkdir', 'rm', 'mv', 'cp', 'edit', 'add', 'diff', 'unstage', 'commit', 'theme', 'pr', 'notifications', 'actions'];
...
<span class="success">Available Commands:</span>
 ...
 <span class="info">Staging (Batch Commit)</span>
  ...
  <span class="info">Notifications (requires login)</span>
     <span class="info">notifications</span>            Show recent notifications
     <span class="info">notifications</span> --all     Show all notifications
     <span class="info">notifications</span> --mark-read   Mark all as read

  <span class="info">Actions (requires login, in repo)</span>
     <span class="info">actions</span>                  List workflow runs
     <span class="info">actions</span> view &lt;run-id&gt;   View run details and jobs
     <span class="info">actions</span> rerun &lt;run-id&gt;  Rerun a workflow
     <span class="info">actions</span> --repo            List repository workflows
